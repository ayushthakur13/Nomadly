...
export const useTripsCache = () => {
  ...
  const refreshTrips = useCallback(async (filters: any = {}) => {
    try { await dispatch<any>(fetchTrips(filters)); } catch (error) { console.error('Failed to fetch trips:', error); }
  }, [dispatch]);
  const refreshTrip = useCallback(async (tripId?: string) => {
    if (!tripId) return;
    try { await dispatch<any>(fetchTripById(tripId)); } catch (error) { console.error(`Failed to fetch trip ${tripId}:`, error); }
  }, [dispatch]);
