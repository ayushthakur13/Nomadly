...
const Navbar = () => {
  ...
  const handleLogoClick = useCallback(() => { navigate(isAuthenticated ? '/home' : '/'); }, [isAuthenticated, navigate]);
  const handleCreateTrip = useCallback(() => { if (isAuthenticated) { dispatch(openCreateTripModal()); } else { navigate('/auth/login'); } setIsMobileMenuOpen(false); }, [isAuthenticated, dispatch, navigate]);
  const scrollToFeatures = useCallback(() => { const section = document.getElementById('features'); section?.scrollIntoView({ behavior: 'smooth' }); setIsMobileMenuOpen(false); }, []);
  const handleLogout = useCallback(async () => { try { await secureLogout(); dispatch(logout()); toast.success('Logged out successfully'); navigate('/', { replace: true }); setIsProfileDropdownOpen(false); } catch (error) { console.error('Logout failed:', error); toast.error('Logout failed. Please try again.'); } }, [dispatch, navigate]);
    ...
    <nav className="w-full sticky top-0 z-50 bg-white/95 backdrop-blur-xl">
      <div className="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8">
        <div className="h-20 flex items-center justify-between">
          ...
          >
            <img src="/images/icon/Nomadly_icon_white-removebg.png" alt="Nomadly" className="w-12 h-12" />
          ...
          <div className="hidden md:flex items-center gap-6 lg:gap-8 text-gray-700">
            {isAuthenticated ? (
              ...
              // Logged out on landing page: Features (scroll) → Explore → About Us
              <>
                <button
              ...
              // Logged out on auth/explore/other pages: Features (link to landing) → Explore → About Us
              <>
          ...
          <div className="hidden md:flex items-center gap-3 flex-shrink-0">
            {isAuthenticated ? (
              ...
              <div className="flex items-center gap-3">
                <Link to="/auth/login" className="px-4 py-2 rounded-lg border border-gray-300 text-gray-700 hover:bg-gray-50 font-medium transition-colors">
                  Log in
                </Link>
                <Link to="/auth/signup" className="px-4 py-2 rounded-lg bg-emerald-600 text-white hover:bg-emerald-700 font-medium transition-colors shadow-sm">
                  Sign up
                </Link>
              </div>
            ) : (
                  ...
                  {isLandingPage ? (
                    ...
                    <>
                      ...
                      <Link
                        ...
                        onClick={() => setIsMobileMenuOpen(false)}
                        className={`px-4 py-2 hover:text-gray-900 hover:bg-gray-50 font-medium transition-colors rounded-lg ${
                          isActiveLink('/aboutus') ? 'text-emerald-600 bg-emerald-50' : 'text-gray-700'
                        }`}
                      >
                        About Us
                      </Link>
